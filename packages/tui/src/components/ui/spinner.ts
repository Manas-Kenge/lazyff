...
/**
 * Color generator function type for spinner animations
 */
...
function createKnightRiderTrail(options: AdvancedGradientOptions): ColorGenerator {
  const { colors: inputColors, defaultColor, enableFading = true, minAlpha = 0 } = options

  // Convert all colors to RGBA
  const colors = inputColors.map((c) => (c instanceof RGBA ? c : RGBA.fromHex(c as string)))
