...
export function MediaInfo({ width = 40 }: MediaInfoProps) {
  ...
  return (
    ...
    >
      ...
      {loading ? (
        <text attributes={TextAttributes.DIM} fg={theme.textMuted}>Loading...</text>
      ) : detailedInfo ? (
        <box flexDirection="column" gap={0}>
          ...
          {detailedInfo.video && (
            <>
              ...
              <InfoRow
                label="Codec"
                value={detailedInfo.video.profile
                  ? `${detailedInfo.video.codec.toUpperCase()} (${detailedInfo.video.profile})`
                  : detailedInfo.video.codec.toUpperCase()
                }
              />
              <InfoRow label="Resolution" value={`${detailedInfo.video.width}×${detailedInfo.video.height}`} />
          ...
          {detailedInfo.audio && (
            <>
              ...
              <InfoRow label="Channels" value={getChannelDescription(detailedInfo.audio.channels, detailedInfo.audio.channelLayout)} />
