...
import { FORMAT_PRESETS, QUALITY_PRESETS } from "../../../ffmpeg/presets.ts"
...
function getFileOptions(cwd: string): AutocompleteOption[] {
  ...
  return files.map((file) => {
    ...
    const relativePath = file.path.startsWith(cwd) ? file.path.slice(cwd.length + 1) : file.path
...
export const Autocomplete = forwardRef<AutocompleteRef, AutocompleteProps>(function Autocomplete(
          ...
          options.map((option, index) => {
            ...
            return (
              <box
                ...
                flexDirection="row"
              >
                <text fg={isSelected ? selectedForeground(theme) : theme.text} flexShrink={0}>
                  {option.display}
                </text>
                {option.description && (
                  <text
