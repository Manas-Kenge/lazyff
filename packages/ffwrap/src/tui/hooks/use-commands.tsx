...
export function useCommands() {
    ...
    try {
      ...
      if (result.success && result.info) {
        const lines = [
          ...
          result.info.format.duration ? `Duration: ${Math.round(result.info.format.duration)}s` : null,
          result.info.video?.codec ? `Codec: ${result.info.video.codec}` : null,
          result.info.video?.width && result.info.video?.height ? `Resolution: ${result.info.video.width}x${result.info.video.height}` : null,
        ].filter(Boolean)
   ...
   */
  const executeCompress = useCallback(async (options: Omit<CompressOptions, "input">) => {
    if (!selectedFile || selectedFile.type === "directory") {
      addMessage({ type: "assistant", content: "No file selected", status: "error" })
      return
    }
   ...
   */
  const executeConvert = useCallback(async (options: { format?: string; quality?: string }) => {
    if (!selectedFile || selectedFile.type === "directory") {
      addMessage({ type: "assistant", content: "No file selected", status: "error" })
      return
    }
   ...
   */
  const executeExtract = useCallback(async (options: Omit<ExtractOptions, "input">) => {
    if (!selectedFile || selectedFile.type === "directory") {
      addMessage({ type: "assistant", content: "No file selected", status: "error" })
      return
    }
   ...
   */
  const executeGif = useCallback(async (options: Omit<GifOptions, "input">) => {
    if (!selectedFile || selectedFile.type === "directory") {
      addMessage({ type: "assistant", content: "No file selected", status: "error" })
      return
    }
   ...
   */
  const executeTrim = useCallback(async (options: Omit<TrimOptions, "input">) => {
    if (!selectedFile || selectedFile.type === "directory") {
      addMessage({ type: "assistant", content: "No file selected", status: "error" })
      return
    }
   ...
   */
  const executeThumbnail = useCallback(async (options: Omit<ThumbnailOptions, "input">) => {
    if (!selectedFile || selectedFile.type === "directory") {
      addMessage({ type: "assistant", content: "No file selected", status: "error" })
      return
    }
   ...
   */
  const executeMerge = useCallback(async (options: { files: FileNode[]; reencode?: boolean }) => {
    if (options.files.length < 2) {
      addMessage({ type: "assistant", content: "Need at least 2 files to merge", status: "error" })
      return
    }
