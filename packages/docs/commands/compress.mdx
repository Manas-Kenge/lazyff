...
```bash
lazyff compress <input> [output] [options]
```
...
lazyff compress video.mp4 small.mp4 --target-size 50MB

# Short form
lazyff compress video.mp4 -s 10MB
```

<Note>
  The actual output size may vary slightly from the target. lazyff calculates the optimal bitrate to hit your target while maintaining quality.
</Note>
...
lazyff compress video.mp4 --bitrate 2M

# 500 Kbps (very small file)
lazyff compress video.mp4 --bitrate 500k

# 5 Mbps (higher quality)
lazyff compress video.mp4 --bitrate 5M
```
...
lazyff compress video.mp4 --percent 25
```
...
lazyff compress video.mp4 -s 10MB --audio-bitrate 64k

# Higher audio quality
lazyff compress video.mp4 -s 50MB --audio-bitrate 192k
```
...
When you specify `--target-size`, lazyff:
...
<Tip>
  ...
  ```bash
  lazyff compress video.mp4 --target-size 24MB
  ```
...
<Tip>
  ...
  ```bash
  lazyff compress video.mp4 --target-size 15MB
  ```
</Tip>
...
lazyff compress video.mp4 -s 24MB -o discord_ready.mp4
```
...
lazyff compress video.mp4 -s 20MB
```
...
lazyff compress video.mp4 --percent 10
```
