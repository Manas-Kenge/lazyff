...
Solutions for common issues when using lazyff.

## Installation Issues

### "Command not found: lazyff"

The lazyff binary is not in your PATH.
...
**Solution:**
```bash
lazyff convert input.mp4 output.mp4 --overwrite
# or
lazyff convert input.mp4 output.mp4 -y
```
...
```bash
lazyff convert input.mp4 -r 720p
lazyff convert input.mp4 -r 1080p
```

Or specify even dimensions:
```bash
lazyff convert input.mp4 -r 1280x720  # Good
lazyff convert input.mp4 -r 1281x721  # Bad - odd numbers
```
...
```bash
# Use lower quality
lazyff convert input.mp4 output.mp4 -q low

# Use compress instead for size control
lazyff compress input.mp4 --target-size 50MB
```
...
lazyff convert input.mp4 output.mp4 -q high
...
lazyff convert input.mp4 output.mp4 --video-codec h264
...
```bash
lazyff merge file1.mp4 file2.mp4 -o output.mp4 --reencode
```
...
lazyff merge file1.mp4 file2.webm -o output.mp4 --reencode

# Option 2: Convert first
lazyff convert file2.webm file2.mp4
lazyff merge file1.mp4 file2.mp4 -o output.mp4
```
...
# Shorter duration
lazyff gif video.mp4 --duration 3
```
...
2. Run `lazyff info input.mp4` to check your file
3. Try the same operation with plain ffmpeg to see detailed errors
4. Open an issue on [GitHub](https://github.com/Manas-Kenge/lazyff/issues)

When reporting issues, include:
- lazyff version (`lazyff version`)
